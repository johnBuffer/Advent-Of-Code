...
def sup_sum(x, l): return sum(l[x] > max(l[i:x]) for i in range(x)) * (1 + sum(l[x] > max(l[x+1:i]) for i in range(x+2, w)))

print(2*(w+h)-4 + sum(is_visible(x, y) for y in range(1, h-1) for x in range(1, w-1)))
print(max(sup_sum(x, d[y]) * sup_sum(y, t[x]) for y in range(1, h-1) for x in range(1, w-1)))
